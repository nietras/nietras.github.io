...
explicit support for these and the code generation the is a bit suboptimal,
given mask register are moved to normal registers each time. And then back.

This should improve with .NET 9, although, it is still the intent not to support
explicit mask register support since this would ballon the number of overloads
needed in the vector API. See [post by Tanner Gooding on mastodon](https://mastodon.social/@tannergooding@tech.lgbt/110758892947524825).
