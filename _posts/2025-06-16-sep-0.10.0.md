...
* Data intentionally fits in L1 cache to be able to measure even small code change impacts
* Single-threaded
...
actions on a GitHub runner, there is higher variance.


Note that all numbers here are single-threaded 
...
site.baseurl }}/2023/08/07/sep-0-2-0/)) and minor code changes. While also
seeing improved performance on new .NET versions. And finally here showing
improvement for going from the AMD 5950X ([Zen
3](https://en.wikipedia.org/wiki/Zen_3)) to AMD 9950X ([Zen
5](https://en.wikipedia.org/wiki/Zen_5)). Hence, this showcases how software
together with hardware improvements can boost performance to the next level. 
...
presented new challanges, as I could neither test/debug the code directly nor
...
However, I still wanted to compare AVX-512 to
...
Let's examine the code and assembly (via
[Disasmo](https://github.com/EgorBo/Disasmo)) for the AVX-512-based parser
