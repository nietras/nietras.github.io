...
class Program
{
    ...
    static void PreloadDotnetDependenciesFromSubdirectoryManually()
    {
        ...
        // First, ensure sub-directory with native libraries is added to dll directories
        ...
        // Then, try manually loading the .NET 6 WPF native library dependencies
    ...
    {
        // NOTE: For the native libraries we load here, we do not care about
        //       closing the library handle since they live as long as the
        //       process.
        var loaded = NativeLibrary.TryLoad(libraryName, Assembly.GetExecutingAssembly(),
